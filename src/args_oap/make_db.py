...
def make_db(file,logger):
    ...
    if subp.returncode == 1: # auto choose dbtype by checking diamond's return code
        if subp.stderr == b'Error: The sequences are expected to be proteins but only contain DNA letters. Use the option --ignore-warnings to proceed.\n' or subp.stderr == b'Error: The sequences are expected to be proteins but only contain DNA letters. Use the option --ignore-warnings to proceed.\r\n':
            subprocess.run(['bwa', 'index', file], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=subprocess.CREATE_NO_WINDOW if os.name=='nt' else 0)
            dbtype = 'nucl'
        else:
    ...
    subprocess.run(['makeblastdb', '-in', os.path.basename(file), '-dbtype', dbtype, '-out', os.path.basename(file)], cwd=os.path.dirname(file), check=False, stdout=subprocess.DEVNULL, creationflags=subprocess.CREATE_NO_WINDOW if os.name=='nt' else 0)
    logger.info('Finished.')

def run_make_db(options):
    make_db(vars(options).get('infile'),vars(options).get('logger'))